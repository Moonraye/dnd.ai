...
- **Two distinct diseases.** *Forgetting* = lossy write path. *Repetition* = a
  ...
  forces it forward.
- **15 messages is too few, but widening alone won't fix it** — it's *which*
...
- **Is "keep detailed context" even the goal?** The Contrarian: *"Detail is the
  enemy."* You need **non-lossy** tokens, not more tokens.
...
- **Append-only ledger grows unbounded** → will need eviction/summarization, but
  for the *right* reason (bounded growth, not lossy-by-default).
...
native multi-turn chat make this *worse*: chat grows the window unboundedly (cost
...
*which* things are remembered, not quantity. A DM's memory is hard facts: names,
